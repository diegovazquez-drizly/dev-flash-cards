...
class BaccaratResultsEngine {
  ...
  calculateOutcome({ playerCards = [], bankerCards = [] }) {
    ...
    else if (difference > 0) return Banker;
    else return Player;
  }
  ...
  calculatePairs({ playerCards = {}, bankerCards = {} }) {
    ...
    if (isPlayerPair && isBankerPair) return BothPair;
    else if (isPlayerPair) return PlayerPair;
    else if (isBankerPair) return BankerPair;
    else return NoPair;
  }
...
export default BaccaratResultsEngine;

export const valueForCard = BaccaratResultsEngine.valueForCard;
